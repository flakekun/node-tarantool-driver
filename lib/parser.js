...
exports._processResponse =  function(buffer, offset, length){
  ...
  else
  {
	...
	if (success) {
		var data;
		if (cmd === tarantoolConstants.RequestCode.rqExecute) {
			data = {
				metadata: _parseSQLMetadata(obj[tarantoolConstants.KeysCode.metadata]),
				data: obj[tarantoolConstants.KeysCode.data]
			};
		} else {
			data = obj[tarantoolConstants.KeysCode.data];
		}
        dfd.resolve(_returnBool(cmd, data));
	} else {
...
function _parseSQLMetadata(response) {
    if (!response) {
        return;
    }

	return response.map(function (item) {
		return {fieldName: item[tarantoolConstants.KeysCode.field_name]};
	});
}