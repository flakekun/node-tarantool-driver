...
Commands.prototype.execute = function(sql, params){
    _this = this;

    return new Promise(function (resolve, reject) {
        var reqId = _this._getRequestId();

        var bufSQL = _this.msgpack.encode(sql);
        var bufParams = _this.msgpack.encode(params || []);

        var len = 12 + bufSQL.length + bufParams.length;
        var buffer = utils.createBuffer(len+5);
        ...
        bufSQL.copy(buffer, 16);
        buffer[16 + bufSQL.length] = tarantoolConstants.KeysCode.sql_bind;
        bufParams.copy(buffer, 17 + bufSQL.length);
